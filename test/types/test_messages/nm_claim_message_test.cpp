...
TEST_F(NMClaimMessageTest, CreateMessage) {
    auto msg = NMClaimMessage::Create(kSrc, kPriority, kNodeCount, kNetworkId);
...
TEST_F(NMClaimMessageTest, SerializationRoundTrip) {
    auto original =
        NMClaimMessage::Create(kSrc, kPriority, kNodeCount, kNetworkId);
...
TEST_F(NMClaimMessageTest, PriorityBoundaryValues) {
    ...
    auto low = NMClaimMessage::Create(kSrc, 0x00, 1, 1);
    ASSERT_TRUE(low.has_value());
    EXPECT_EQ(low->GetPriority(), 0x00);

    // Max priority (lowest priority)
    auto high = NMClaimMessage::Create(kSrc, 0xFF, 0, 0xFFFF);
