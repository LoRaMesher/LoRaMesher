...
TEST(MessageTest, SerializationTest) {
   ...
   EXPECT_EQ(memcmp(data + BaseHeader::size(), payload.data(), payload.size()), 0);
}
...
TEST(MessageTest, RoutingSerializationTest) {
    ...
    ASSERT_EQ(msg.getTotalSize(), RoutingHeader::size() + BaseHeader::size() + payload.size());
    ...
    // Expected memory layout:
    // [2B dest][2B src][1B type][1B size][2B nextHop][1B seqId][2B number][payload]
    ASSERT_EQ(serialized.size(), RoutingHeader::size() + BaseHeader::size() + payload.size());
    ...
    // Check payload
    EXPECT_EQ(memcmp(data + RoutingHeader::size() + BaseHeader::size(), payload.data(), payload.size()), 0);
}
